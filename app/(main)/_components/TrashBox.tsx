...
const TrashBox = () => {
  ...
  const onRestore = (e: any, documentId: Id<"documents">)=>{
    e.stopPropagation();
        ...
        <div className='mt-2 px-1 pb-1'>
            <p className='hidden last:block text-xs text-center text-muted-foreground'>No documents found</p>
            {
                filterDocuments?.map((d)=>(
                    <div key={d._id} role='button' onClick={()=>onClick(d._id)} className='text-sm rounded-sm w-full hover:bg-primary/5 flex items-center text-primary justify-between'>
